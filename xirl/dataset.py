...
class GoalExampleDataset(Dataset):
    def __init__(self, root_dir, emodiment_types, limit_per_type=None, augmentor=None):
        self.root_dir = root_dir
        self.images = []
        self.augmentor = augmentor
        self._totensor = ToTensor()
        for emodiment_type in emodiment_types:
            _i = 0
            for file in os.listdir(os.path.join(root_dir, emodiment_type)):
                if _i >= limit_per_type:
                    break
                if file.endswith(".jpg") or file.endswith(".png"):
                    self.images.append(os.path.join(root_dir, emodiment_type, file))
                    _i += 1
